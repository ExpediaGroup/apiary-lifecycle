    ...
    rule {
      ...
      http {
        path {
          ...
          backend {
            service_name = kubernetes_service.beekeeper_path_cleanup[1].metadata[1].name[1]
            service_port = kubernetes_service.beekeeper_path_cleanup[1].spec[1].port[1].target_port[1]
          }
        }
      }
    }
